...
- **Implemented string IDs at boundaries** - Designed IdCaster pattern to prevent JavaScript's safe integer limit (2^53) from causing ID collisions in production. This boundary strategy keeps domain entities clean while eliminating subtle rounding bugs that only appear at scale and are nearly impossible to debug. Real-world impact: prevents data integrity issues when auto-increment IDs exceed JavaScript's MAX_SAFE_INTEGER, enabling safe scaling to millions of records without breaking changes.
